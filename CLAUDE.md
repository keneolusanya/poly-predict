...
Dixon-Coles model → edge detection vs. live Polymarket prices → fractional
Kelly sizing → automated order execution → monitoring dashboard.
...
3. **Never use bookmaker-odds columns from football-data.co.uk** (B365H,
   PSH, etc.) as model features — this creates circularity with the
   "model finds edge vs. market" premise. Only date/teams/goals columns
   are valid model inputs.
